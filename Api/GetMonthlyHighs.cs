...
namespace Api
{
    public class GetMonthlyHighs
    {
        private readonly ILogger<GetMonthlyHighs> _logger;
        private readonly IMemoryCache _cache;
        private readonly IGetMonthlyHighsRepository _repository;

        public GetMonthlyHighs(ILogger<GetMonthlyHighs> log, 
            IMemoryCache cache,
            IGetMonthlyHighsRepository repository)
        {
            _logger = log;
            _cache = cache;
        ...
        [OpenApiResponseWithBody(statusCode: HttpStatusCode.OK, contentType: "text/plain", 
            bodyType: typeof(GetMonthlyHighsResponse), Description = "The OK response")]
        ...
        [OpenApiResponseWithBody(statusCode: HttpStatusCode.NotFound, contentType: "text/plain",
            bodyType: typeof(string), Description = "No monthly highs could be found for this symbol")]
            ...
            [HttpTrigger(AuthorizationLevel.Function, "get", Route = null)] HttpRequest req)
        {
            ...
            if (_cache.TryGetValue($"GetMonthlyHighs-{symbol}", out var cachedMatches)
                && cachedMatches is GetMonthlyHighsResponse)
                return new OkObjectResult(cachedMatches);

            var response = await _repository.GetMonthlyHighs(symbol);
            //var response = _getStockMatchesResponseFactory.CreateStockInformation(results);
            if (response.MonthlyHighs.Any())
            {
                _cache.Set($"GetMonthlyHighs-{symbol}", response, TimeSpan.FromMinutes(5));
